...
export function FloorMap({ floor, onDeskClick, onRoomClick, selectedDeskId, highlightAvailable, date, previewMode }: FloorMapProps) {
  ...
  const [dimensions, setDimensions] = useState({ width: 336, height: 268 });
  ...
  const scale = Math.min(dimensions.width / mapW, dimensions.height / mapH);
                ...
                {room.type === 'server_room' && (
                  <div className="w-[85%] h-[80%] bg-slate-800 dark:bg-slate-900 border-2 border-slate-700 dark:border-slate-800 rounded p-1.5 shadow-inner flex flex-col justify-between gap-1 relative z-10">
                    {Array.from({ length: 3 }).map((_, i) => (
                      <div key={i} className="h-2 bg-slate-750 dark:bg-slate-855 rounded flex items-center justify-between px-1.5 border border-slate-700/50">
            ...
            >
              ...
              {occupant ? (
                <>
                  ...
                  <div
                    className={cn(
                      "absolute left-1/2 -translate-x-1/2 px-1 py-0.5 rounded bg-slate-900/95 dark:bg-slate-955/95 text-white text-[7px] font-bold tracking-tight whitespace-nowrap shadow-sm border border-slate-700/30 max-w-[54px] truncate leading-none z-30",
  ...
  return (
    <div className={cn("flex h-full flex-col", previewMode ? "" : "gap-3")}>
      {!previewMode && (
        <>
          <div className="flex flex-wrap items-center gap-2 sm:gap-3 w-full">
            <div className="flex items-center gap-1 rounded-lg border border-gray-200 bg-white p-1 shadow-sm dark:border-gray-805 dark:bg-gray-955">
              ...
              <span className="min-w-[3rem] px-1 text-center text-xs font-bold text-gray-700 dark:text-gray-250">{Math.round(zoom * 100)}%</span>
            ...
            <div className="sm:ml-auto flex items-center gap-1 rounded-lg border border-gray-200 bg-white p-1 shadow-sm dark:border-gray-808 dark:bg-gray-955 w-full sm:w-auto justify-between sm:justify-start">
      ...
      <div
        className={cn(
          "relative min-h-[340px] flex-1 overflow-hidden p-5 shadow-inner",
          previewMode 
            ? "rounded-none border-0 bg-transparent p-2 min-h-0 h-full w-full select-none pointer-events-none flex items-center justify-center" 
            : "cursor-grab rounded-xl border border-slate-200 bg-[#eef7f8] md:min-h-[560px] active:cursor-grabbing dark:border-gray-800 dark:bg-slate-950"
        )}
        style={{ touchAction: 'none' }}
        onMouseDown={previewMode ? undefined : handleMouseDown}
        onMouseMove={previewMode ? undefined : handleMouseMove}
        onMouseUp={previewMode ? undefined : () => { isPanning.current = false; }}
        onMouseLeave={previewMode ? undefined : () => { isPanning.current = false; }}
        onTouchStart={previewMode ? undefined : handleTouchStart}
        onTouchMove={previewMode ? undefined : handleTouchMove}
        onTouchEnd={previewMode ? undefined : () => { isPanning.current = false; }}
        onWheel={previewMode ? undefined : (e) => {
          e.preventDefault();
      ...
      >
        {previewMode ? (
          <div
            ref={containerRef}
            className="w-full h-full overflow-hidden relative flex items-center justify-center bg-transparent"
          >
            <div
              className="overflow-hidden relative rounded-2xl border border-gray-250 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-md transition-all duration-300"
              style={{
                width: mapW * scale,
                height: mapH * scale,
              }}
            >
              <div
                className="office-plan animate-fade-in absolute top-0 left-0"
                style={{
                  width: mapW,
                  height: mapH,
          ...
          <div
            className="office-plan animate-fade-in"
            style={{
              width: mapW,
              height: mapH,
