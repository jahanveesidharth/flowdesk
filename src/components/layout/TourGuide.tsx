...
export function TourGuide() {
  ...
  useEffect(() => {
    if (!isActive || !highlightRect || showCompletion) return;
  ...
  }, [isActive, currentStep, highlightRect, showCompletion, steps]);
  ...
  const getTooltipStyle = () => {
    ...
    const tooltipHeight = 200; // Estimated max height for safety clamping
    ...
    // 1. Initial Positioning based on step preferred placement
    if (step.position === 'bottom') {
      targetTop = bottom + scrollY + gap;
      targetLeft = left + scrollX + width / 2 - tooltipWidth / 2;
      
      // If it overflows viewport bottom, try to place at the top of the element instead
      if (targetTop + tooltipHeight > viewportHeight + scrollY - safetyMargin) {
        const tryTop = top + scrollY - tooltipHeight - gap;
        if (tryTop > scrollY + safetyMargin) {
          targetTop = tryTop;
        }
      }
    } else if (step.position === 'top') {
      targetTop = top + scrollY - tooltipHeight - gap;
      targetLeft = left + scrollX + width / 2 - tooltipWidth / 2;
      
      // If it overflows viewport top, try to place at the bottom of the element instead
      if (targetTop < scrollY + safetyMargin) {
        const tryBottom = bottom + scrollY + gap;
        if (tryBottom + tooltipHeight < viewportHeight + scrollY - safetyMargin) {
          targetTop = tryBottom;
        }
      }
    } else if (step.position === 'right') {
      targetTop = top + scrollY + height / 2 - tooltipHeight / 2;
      targetLeft = right + scrollX + gap;
      
      // If it overflows viewport right, try to place on the left instead
      if (targetLeft + tooltipWidth > viewportWidth - safetyMargin) {
        const tryLeft = left + scrollX - tooltipWidth - gap;
    ...
    const maxLeft = scrollX + viewportWidth - tooltipWidth - safetyMargin;
    if (targetLeft < minLeft) targetLeft = minLeft;
    if (targetLeft > maxLeft) targetLeft = maxLeft;

    const minTop = scrollY + safetyMargin;
    const maxTop = scrollY + viewportHeight - tooltipHeight - safetyMargin;
  ...
  return (
    <>
      ...
      <div 
        className="fixed inset-0 bg-black/60 dark:bg-black/75 z-[9998] transition-opacity duration-300 pointer-events-auto"
        onClick={(e) => e.stopPropagation()}
      >
        {highlightRect && !isRouting && (
          <div
            className="absolute rounded-xl border-2 border-brand-500 bg-transparent transition-all duration-300 pointer-events-none"
            style={{
              top: `${highlightRect.top + window.scrollY - 4}px`,
              left: `${highlightRect.left + window.scrollX - 4}px`,
      ...
      {!showCompletion && (
        <div
          ...
          className="bg-white/95 dark:bg-gray-900/95 border border-gray-200/80 dark:border-gray-800 shadow-2xl rounded-2xl p-5 backdrop-blur-md transition-all duration-300 max-h-[calc(100vh-40px)] overflow-y-auto"
        >
          {isRouting ? (
            ...
            <>
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                <button
                  onClick={handleSkip}
                  className="p-1 rounded-lg text-gray-400 hover:text-gray-655 dark:hover:text-gray-250 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
                  title="Skip Tour"
                >
              ...
              {/* Title & Body */}
              <h4 className="text-sm font-extrabold text-gray-900 dark:text-white mb-1.5">
              ...
              <div className="flex items-center justify-between border-t border-gray-100 dark:border-gray-800/80 pt-3">
                <button
                  onClick={handleSkip}
                  className="text-[11px] font-bold text-gray-400 dark:text-gray-500 hover:text-gray-655 dark:hover:text-gray-300 transition-colors"
                >
                  Skip Tour
                </button>
                ...
                <div className="flex items-center gap-2">
                  {currentStep > 0 && (
                    <button
                      onClick={handleBack}
                      className="px-2.5 py-1.5 rounded-lg border border-gray-200 dark:border-gray-800 hover:bg-gray-55 dark:hover:bg-gray-800 text-[11px] font-bold text-gray-700 dark:text-gray-300 flex items-center gap-1 transition-all"
                    >
                  ...
                  <button
                    onClick={handleNext}
                    className="px-3 py-1.5 rounded-lg bg-brand-500 hover:bg-brand-600 text-white text-[11px] font-bold flex items-center gap-1 shadow-sm transition-all"
                  >
      ...
      {/* Completion Modal */}
      {showCompletion && (
        <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 bg-black/10 backdrop-blur-xs">
          <div className="bg-white dark:bg-gray-900 border border-gray-150 dark:border-gray-800 rounded-3xl p-8 max-w-sm w-full shadow-2xl text-center relative overflow-hidden animate-scale-in">
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 leading-relaxed mb-6">
              You are now fully set up to explore the workstation planner, schedule bookings, and coordinates team attendance.
            </p>
            ...
            <div className="space-y-3">
              <button
                ...
                className="w-full py-2.5 rounded-xl bg-brand-500 hover:bg-brand-600 text-white text-xs font-bold transition-all shadow-sm flex items-center justify-center gap-1.5"
              >
              ...
              <button
                onClick={endTour}
                className="w-full py-2.5 rounded-xl bg-gray-55 hover:bg-gray-100 dark:bg-gray-800 dark:hover:bg-gray-750 text-gray-700 dark:text-gray-200 text-xs font-bold transition-all border border-gray-200/50 dark:border-gray-700/50"
              >
              ...
              <button
                ...
                className="w-full py-2.5 rounded-xl bg-rose-50 hover:bg-rose-100/80 dark:bg-rose-950/10 dark:hover:bg-rose-950/20 text-rose-600 dark:text-rose-400 text-xs font-bold transition-all flex items-center justify-center gap-1.5"
              >
