...
export function Tabs({ tabs, activeTab, onChange, variant = 'underline', className }: TabsProps) {
  if (variant === 'pills') {
    return (
      <div className={cn('flex gap-1 p-1 bg-gray-150/60 dark:bg-gray-900/60 rounded-xl border border-gray-200/40 dark:border-gray-800/40 overflow-x-auto whitespace-nowrap scrollbar-none', className)}>
        {tabs.map(tab => (
          <button
            key={tab.id}
            onClick={() => onChange(tab.id)}
            className={cn(
              'flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-semibold transition-all flex-1 justify-center shrink-0',
              activeTab === tab.id 
  ...
  if (variant === 'boxed') {
    return (
      <div className={cn('flex gap-0 border border-gray-200 dark:border-gray-800 rounded-xl overflow-hidden shadow-sm overflow-x-auto whitespace-nowrap scrollbar-none', className)}>
        {tabs.map((tab, i) => (
          <button
            key={tab.id}
            onClick={() => onChange(tab.id)}
            className={cn(
              'flex items-center gap-1.5 px-4 py-2.5 text-sm font-semibold transition-all flex-1 justify-center shrink-0',
  ...
  return (
    <div className={cn('flex gap-6 border-b border-gray-200 dark:border-gray-800 overflow-x-auto whitespace-nowrap scrollbar-none', className)}>
      {tabs.map(tab => (
        <button
          key={tab.id}
          onClick={() => onChange(tab.id)}
          className={cn(
            'flex items-center gap-1.5 pb-3.5 text-sm font-semibold border-b-2 transition-all -mb-px shrink-0',
            activeTab === tab.id
